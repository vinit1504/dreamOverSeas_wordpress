...
class Envira_Serialization_Admin {
	...
	public function fix_serialized_string( $string ) {
		...
		if ( @unserialize( $string ) !== false ) { // @codingStandardsIgnoreLine
			return @unserialize( $string ); // @codingStandardsIgnoreLine
		}

		// String needs fixing - fix it.
		$string = preg_replace_callback( '/\bs:(\d+):"(.*?)"/', array( $this, 'fix_str_length' ), $string );

		return unserialize( $string ); // @codingStandardsIgnoreLine - !!! TODO migrate to WP filesystem
